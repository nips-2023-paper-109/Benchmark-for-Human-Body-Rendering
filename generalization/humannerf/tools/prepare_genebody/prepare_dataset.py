...
def main(argv):
    ...
    cam_Ks = np.array([cams['{:02d}'.format(view)]['K'] for view in select_view]).astype('float32')
    cam_Rs = np.array([cams['{:02d}'.format(view)]['c2w_R'] for view in select_view]).astype('float32')
    cam_Ts = np.array([cams['{:02d}'.format(view)]['c2w_T'] for view in select_view]).astype('float32')
    cam_Ds = np.array([cams['{:02d}'.format(view)]['D'] for view in select_view]).astype('float32')
    ...
    img_paths = [[os.path.join('{:02d}'.format(view_i), im_i) for view_i in select_view] for im_i in ims_0]
    ...
    for idx_frame, path_frame in enumerate(tqdm(img_paths)):
        for idx_camera, path_camera in enumerate(path_frame):
            real_idx_frame = idx_frame + start_idx
            real_idx_camera = select_view[idx_camera]
            out_name = 'frame_{:06d}_camera_{:02d}'.format(real_idx_frame, real_idx_camera)
