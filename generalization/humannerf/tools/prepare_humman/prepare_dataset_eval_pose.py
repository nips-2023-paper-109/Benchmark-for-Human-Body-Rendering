...
def main(argv):
    ...
    start_idx = cfg['start_idx']+ max_frames * 6
    ...
    total_views = cams_keys

    select_view = []

    train_views = cfg['training_view']

    select_view = [view for view in range(len(total_views)) if view not in train_views]
    ...
    ims_0 = sorted(os.listdir(os.path.join(subject_dir, 'kinect_color' ,'kinect_000')), key=lambda x: int(x[:-4]))[max_frames::skip]
    ...
    output_path = os.path.join(cfg['output']['dir'],  subject+'_eval_pose')
    ...
    for idx_frame, path_frame in enumerate(tqdm(img_paths)):
        for idx_camera, path_camera in enumerate(path_frame):
            real_idx_frame = idx_frame * skip * 6 + start_idx
