...
class Network(nn.Module):
    ...
    def _apply_mlp_kernals(
        ...
        for i in range(0, pos_flat.shape[0], chunk):
            ...
            ## xyz: 3d points coordinates after skeletal transformation to canonical space

            if not cfg.ignore_non_rigid_motions:
                non_rigid_embed_xyz = non_rigid_pos_embed_fn(xyz)

                ## T_NR(T_rigid(xyz, pose), pose)
                ## T_rigid(xyz, pose) = non_rigid_embed_xyz
                ## result['xyz'] = xyz + offset
            ...
            ## xyz_cnl = T(xyz) = T_rigid(xyz, pose) + T_NR(T_rigid(xyz, pose), pose)

            xyz_embedded = pos_embed_fn(xyz)

            ## color, sigma = F(xyz_cnl)
            raws += [self.cnl_mlp(
    ...
    def _render_rays(
        ...
        ## pts.shape = [2400, 128, 3]
        pts = rays_o[...,None,:] + rays_d[...,None,:] * z_vals[...,:,None]
        
        ## motion_scale_Rs[0].shape = [24, 3, 3]
        ## motion_Ts[0].shape = [24, 3]
        ## motion_weights_vol.shape = [25, 32, 32, 32]

        ## 利用canonical space 下定义的motion fields将每一个pts转换到canonical space的Tpose上
        ## 对应论文的skeletal motion计算
        ...
        ## cnl_pts: 3d points coordinates after skeletal transformation
    ...
    def forward(self,
        ...
        ## KK： 计算当前pose与canonical pose的相对关系
        motion_scale_Rs, motion_Ts = self._get_motion_base(
                                            dst_Rs=dst_Rs, 
                                            dst_Ts=dst_Ts, 
                                            cnl_gtfms=cnl_gtfms)

        ## KK: motion_weights_priors是根据canonical pose初始化的volume
        ## self.mweight_vol_decoder是3d convolution模块
