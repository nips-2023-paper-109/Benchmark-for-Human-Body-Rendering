...
class Network(nn.Module):
    ...
    def forward(self, wpts, viewdir, dists, batch):
        ...
        with torch.no_grad():
            ...
            ### KK ###
            ## pose_pts = torch.Size([1, 65536, 3])
            ## cfg.norm_th = 0.05
            ## pind.shape = torch.Size([1, 65536])
            ## init_pbw.shape = torch.Size([1, 25, 65536])
            ## batch['pbw'].shape = torch.Size([1, 69, 75, 25, 25])
            ## batch['pbounds'] = tensor([[[-0.8543, -1.1956, -0.1768],
            ##                             [ 0.8597,  0.6135,  0.3317]]], device='cuda:0')
