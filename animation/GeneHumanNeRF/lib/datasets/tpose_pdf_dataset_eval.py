...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        ...
        self.ref_dataset_path = data_root
        
        root_part = data_root.split('/')

        data_root = '/'.join([root_part[0], root_part[1]+'_eval', root_part[3]+'_eval'])
        ...
        # self.ref_dataset_path = '/'.join([root_part[0], root_part[1]+'_eval', root_part[2]+'_eval'])

        N_inputs = 10
        skip = cfg.num_train_frame//N_inputs
        self.ref_ims = self.load_ref_train_frames()[::skip][:N_inputs]
        print(self.ref_ims)

        self.ref_cameras = self.load_ref_cameras()
        self.ref_mesh_infos = self.load_ref_mesh_infos()
    ...
    def load_ref_frames(self):
        ...
        for frame_name in self.ref_ims:
            img_path = os.path.join(self.ref_dataset_path, 'images', '{}.png'.format(frame_name))
            img = imageio.imread(img_path).astype(np.float32) / 255.
            H, W = int(img.shape[0] * cfg.ratio), int(img.shape[1] * cfg.ratio)
            
            mask_path = os.path.join(self.ref_dataset_path, 
                                'masks', 
            ...
            vis_map_path = os.path.join(self.ref_dataset_path, 
                                'vis_maps', 
                                '{}.npy'.format(frame_name))

            vis_map = np.load(vis_map_path)

            wpts, pvertices, A, Rh, Th, poses, nearest_frame_index = self.prepare_input(frame_name, ref=True)

            ref_camera = self.ref_cameras[frame_name]
    ...
    def load_ref_mesh_infos(self):
        mesh_infos = None
        with open(os.path.join(self.ref_dataset_path, 'mesh_infos.pkl'), 'rb') as f:   
            mesh_infos = pickle.load(f)
    ...
    def load_ref_cameras(self):
        cameras = None
        with open(os.path.join(self.ref_dataset_path, 'cameras.pkl'), 'rb') as f: 
            cameras = pickle.load(f)
        return cameras
    ...
    def load_ref_train_frames(self):
        img_paths = list_files(os.path.join(self.ref_dataset_path, 'images'),
                               exts=['.png'])
        return [split_path(ipath)[1] for ipath in img_paths]
    ...
    def query_ref_dst_skeleton(self, frame_name):
        return {
            'poses': self.ref_mesh_infos[frame_name]['poses'].astype('float32'), ## (72,)
            'dst_tpose_joints': \
                self.ref_mesh_infos[frame_name]['tpose_joints'].astype('float32'), ## (24, 3)
            'bbox': self.ref_mesh_infos[frame_name]['bbox'].copy(),
            'Rh': self.ref_mesh_infos[frame_name]['Rh'].astype('float32'),
            'Th': self.ref_mesh_infos[frame_name]['Th'].astype('float32'),
            'betas': self.ref_mesh_infos[frame_name]['betas'].astype('float32'),
            'vertices': self.ref_mesh_infos[frame_name]['vertices'].astype('float32')
        }
    ...
    def prepare_input(self, frame_name, ref=False):
        if ref:
            dst_skel_info = self.query_ref_dst_skeleton(frame_name)
        else:
    ...
    def __getitem__(self, index):
        ...
        cam_ind = int(frame_name.split('_')[-1])
