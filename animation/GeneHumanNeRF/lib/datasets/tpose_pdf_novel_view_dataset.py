...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        ...
        frame_name = self.ims[i]

        E = self.cameras[frame_name]['extrinsics'].astype(np.float32).copy()
        ...
        for theta in np.linspace(0., 2 * np.pi, cfg.render_views + 1)[:-1]:
            rot_mat = cv2.Rodrigues(rot_axis*theta)[0]
            mat = ref_cam.copy()
            mat[:3, :3] = np.dot(ref_cam[:3, :3], rot_mat)
            # mat[:3] = np.dot(rot_mat, ref_cam[:3])
    ...
    def load_ref_frames(self):
        ...
        return {'ref_imgs': np.stack(ref_imgs, axis=0),
    ...
    def __getitem__(self, index):
        ...
        frame_index = cfg.begin_ith_frame

        frame_name = self.ims[frame_index]
