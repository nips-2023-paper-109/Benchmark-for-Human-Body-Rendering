...
subjects_list = {'people_snapshot': ['female-1-casual',
                                    'female-3-sport',
                                    'female-4-casual',
                                    'female-6-plaza',
                                    'female-8-plaza',
                 ...
                 'zju_mocap': ['313', '315', '377', '386', '390', '392', '396']
                        }

bad_seq_data =  ['p000590_a000071']

class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        super(Dataset, self).__init__()

        if data_root.split('/')[-1] in subjects_list:
            self.subjects = subjects_list[data_root.split('/')[-1]]
        else:

            assert data_root.split('/')[-1] == 'humman123'

            seq_data_all = os.listdir(os.path.join(data_root, 'sequences'))

            train_seq = []
            with open(os.path.join(data_root, 'train.txt'), 'r') as f:
                for line in f.readlines():
                    line = line.strip('\n')
                    if line in seq_data_all and line not in bad_seq_data:
                        train_seq.append(line)
            
            self.subjects = train_seq

            data_root = os.path.join(data_root, 'sequences')

            # self.subjects = os.listdir(data_root)
        ...
        self.sample_mode = 'pixel' ## 'pixel' or 'patch'
        print("Sample mode: ", self.sample_mode)
        ...
        for subject in self.subjects:
            ...
            skip = num_train_frames//self.N_inputs
            self.ref_ims = self.ims[::skip][:self.N_inputs]
            # self.ref_infos = self.load_ref_frames()
            if cfg.random_input:
                print("*** Random Input Frames... ***")
            else:
                print("*** Fixed Input Frames... ***")
            ...
            self.total_ref_ims[subject] = self.ref_ims
    ...
    def __getitem__(self, index):
        ...
        if cfg.random_input:
            self.ref_ims = np.random.choice(self.ims, size=self.N_inputs)
        else:
            self.ref_ims = self.total_ref_ims[subject]

        self.ref_infos = self.load_ref_frames()

        self.ref_infos.update({'faces': self.smpl_model.faces})
        ...
        if self.sample_mode == 'pixel':
            ...
            ray_o, ray_d, rgb, near, far, \
                target_patches, patch_masks, patch_div_indices, occupancy = if_nerf_dutils.sample_patch_rays(
                    img, msk, K, R, T, wbounds)

            occupancy = occupancy.astype(np.uint8)

            mask_at_box = np.ones(ray_o.shape[0]) > 0
