...
def sample_patch_rays(img, msk, K, R, T, bounds, N_patches=6, patch_size=20):
    ...
    pose = np.concatenate([R, T], axis=1)
    bbox_mask = get_bound_2d_mask(bounds, K, pose, H, W)
    ...
    ray_mask = bbox_mask.reshape(-1)
    ray_img = img.reshape(-1, 3)[ray_mask]
    rays_o = rays_o.reshape(-1, 3)[ray_mask]
    rays_d = rays_d.reshape(-1, 3)[ray_mask]

    occupancy = msk.reshape(-1)[ray_mask]

    near, far, mask_at_box = get_near_far(bounds, rays_o, rays_d)

    ray_img = ray_img[mask_at_box]
    rays_o = rays_o[mask_at_box]
    rays_d = rays_d[mask_at_box]
    occupancy = occupancy[mask_at_box]

    ray_mask_ = np.zeros_like(ray_mask)
    ray_mask_[ray_mask] = mask_at_box
    ray_mask = ray_mask_
    # ray_mask = ray_mask[mask_at_box]
    ...
    occupancy = occupancy[select_inds]
    ...
    return rays_o, rays_d, ray_img, near, far, \
            target_patches, patch_masks, patch_div_indices, occupancy
